...
export class InstalacionComponent implements OnInit {
    ...
    /**
     * Constructor de la clase Instalacion (MODIFICACION)
     * @param toast
     ...
     */
    cargarDatos(){
        this.modificacionService.getAllInstalacionesModificacion().subscribe((response) => {
    ...
    cargarDatosInstalacion(){
        ...
        this.columns = [
            { text: 'Dirección', datafield: 'direccion', width:'50%' },
            { text: 'Tipo', datafield: 'tipo_almacen', width:'35%' },
            {
                text: 'Estado',
    ...
    eliminarInstalacion(){
        if (this.dataselect.id>0 && this.dataselect.fecha_aprobacion == null){
            ...
            .then((result) => {
                if (result.value) {
                    this.modificacionService.deleteModificacionInstalacion(this.dataselect.id).subscribe((response) => {
     ...
     */
    registrarModificacionInstalacion(formInstalacion: NgForm){
        this.modificacionService.saveSolicitudModificacionInstalacion(formInstalacion.value).subscribe((response) => {
            if (response.type=='success'){
